...
func ProfileRoutes(db *sql.DB) http.Handler {
	relationsHandler := profileRelations(db)
	profileHandler := profileView(db)
...
func profileView(db *sql.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func ProfilePrivacy(db *sql.DB) http.Handler {
	type request struct {
